...
- (NSDictionary *)requestHTTPBody {
    ...
    if (self.aid) {
        return @{@"name":self.recipientName, @"p":self.province,@"n":self.city,@"c":self.area,@"address":self.addressString,@"mobile":self.mobile,@"zipcode":self.zipcode,@"is_delault":self.is_default ? @"1" : @"0" ,@"aid":[NSNumber numberWithFloat:self.aid]};
    } else {
